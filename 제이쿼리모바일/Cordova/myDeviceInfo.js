...
// 단말기 배터리
function  getBattery() {
	window.addEventLister("battery", function(info){
...
function getConnection(){
  ...
  $('#resultArea').html("<b>네트워크 연결 상태 : " + status + "</p>");
};
...
function getLocation() {
  ...
  function(Position){
    $('#resultArea').html("<p>위도 : " + Position.coords.latitude+ "</p>"
                         + "<p>경도 : " + Position.coords.longtitude + "</p>");
  }, function(posError){
    $('#resultArea').html("<p>위치 정보 오류: (" + posError.code + ")" + posError.message +"</p>");
  },
  {maxImumaAge: 3000, timeout : 50000, enableHighAccuracy: true}
  );
};
